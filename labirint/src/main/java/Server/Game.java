...
public class Game {
    ...
    private boolean optionsPassed = false;
    int rulescount;
    ...
    public void notifyRules() throws IOException {
        ...
        for (Player p : players) {
            p.send(s);
            System.out.println("Игроку " + p.getPlayerId() + "Отправлены правила");
            p.send("( userscount " + players.size() + " )");
            System.out.println("Игрок " + p.getPlayerId() + "Получил кол-во игроков");
        }
    }
